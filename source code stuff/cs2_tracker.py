...
def _fetch_all(steam_id, csfloat_key, steam_cookie, buff_cookie, fid):
    """Main fetch worker — runs in a background thread."""
    ...
    def fetch_one(pair):
        ...
        diff = diff_pct = None
        if steam_price and steam_price > 0 and cf_data and cf_data.get("price"):
...
def _fetch_inventory(steam_id, steam_cookie, fid):
    ...
    while True:
        ...
        try:
            ...
            _push("error", "Steam returned invalid data. Try again in a moment.")
            return None
